...
from preProcessingImages import fuzzy_bgr
from preProcessingImages import segment_roi
from preProcessingImages import sick_melon
...
def detect_melon(image):
	...
	try:
		...
		try:
			ind = fuzzy_bgr.fuzzy_ripe_index(b, g, r)
		...
		cv2.imwrite("preProcessingImages/output_images/melon.png",melon)
		cv2.imwrite("preProcessingImages/output_images/Final.png",fin)
		cv2.imwrite("preProcessingImages/output_images/melon_mask.png",melon_mask)
		cv2.imwrite("preProcessingImages/output_images/roi_fin.png",roi_fin)
...
n = 7
img = 'test/sick/{}.jpg'.format(n)
Confidence, index, blue, green, red, m, f, mask, roi_f = detect_melon(img)

cv2.imshow("Melon", m)
#cv2.imshow("Skin sample", preparing_bgr)

cv2.imwrite('test/sick/roi{}.jpg'.format(n), m)
cv2.imwrite('test/sick/roi_f{}.jpg'.format(n), roi_f)

cv2.imshow('Final Result', f)
