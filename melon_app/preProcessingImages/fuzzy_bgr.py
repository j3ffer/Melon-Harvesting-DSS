...
def fuzzy_ripe_index(blue, green, red):
	...
	plt.savefig('preProcessingImages/output_images/blue_mf.png')
	plt.close()

	Green.view(sim=indexing)
	plt.savefig('preProcessingImages/output_images/green_mf.png')
	plt.close()

	Red.view(sim=indexing)
	plt.savefig('preProcessingImages/output_images/red_mf.png')
	plt.close()

	Index.view(sim=indexing)
	plt.savefig('preProcessingImages/output_images/index.png')
	plt.close()
	#except ValueError:
...
index = fuzzy_ripe_index(104, 211, 227);
print(index)

#n.view(sim=indexing)	
#clear_output(n.view(sim=indexing))
#plt.savefig('output_images/Fuzzy Output/g.png')
"""
